# 🌳 YGGDRASIL {: #yggdrasil-main }

> *Ein selbst aufgebautes digitales Ökosystem aus Servern, Netzwerken, Sicherheit und Automatisierung.*

---

## 🌍 Die neun Welten

### ⚔️ ASGARD — Die Fundamente
Die heilige Halle der Kernsysteme. Hier residieren die Schmieden, die allen anderen Welten Stabilität verleihen.

[Proxmox VE](ASGARD/proxmox.md){: .world-link } · [Ubuntu Server](ASGARD/ubuntu-server.md){: .world-link } · [Docker](ASGARD/docker.md){: .world-link } · [Storage](ASGARD/storage.md){: .world-link }

---

### 👁️ HEIMDALL — Der Wächter
Der allsehende Wächter am Horizont. Nichts entgeht seinem Blick — weder im Licht noch im Dunkel des Netzwerks.

[Wazuh SIEM](HEIMDALL/wazuh.md){: .world-link } · [Suricata IDS](HEIMDALL/suricata.md){: .world-link } · [EveBox](HEIMDALL/evebox.md){: .world-link } · [YARA](HEIMDALL/yara.md){: .world-link }

---

### 🌈 BIFROST — Die Brücke
Das schimmernde Regenbogen-Tor zwischen den Welten. Die Pforte, die den Verkehr lenkt und den Weg weist.

[OPNsense](BIFROST/opnsense.md){: .world-link } · [VLAN](BIFROST/vlan.md){: .world-link } · [Routing](BIFROST/routing.md){: .world-link } · [DNS & DHCP](BIFROST/dns-dhcp.md){: .world-link }

---

### 🏠 MIDGARD — Die sichtbare Welt
Die Welt der Sterblichen — hier leben die Dienste, die das Netzwerk beleben und die Welten senden.

[AzuraCast](MIDGARD/azuracast.md){: .world-link } · [Gitea](MIDGARD/gitea.md){: .world-link } · [Passbolt](MIDGARD/passbolt.md){: .world-link } · [Mailserver](MIDGARD/mailserver.md){: .world-link }

---

### 🧠 HUGINN & MUNINN — Das Wissen
Odins Raben, die über die Welt fliegen und Wissen sammeln. Alles lokal, alles unter meiner Kontrolle.

[Ollama](HUGINN-MUNINN/ollama.md){: .world-link } · [Open WebUI](HUGINN-MUNINN/open-webui.md){: .world-link } · [Automatisierung](HUGINN-MUNINN/automation.md){: .world-link } · [Experimente](HUGINN-MUNINN/experiments.md){: .world-link }

---

### 📜 ODIN — Die Weisheit
Der Allvater der Verwaltung. Hier ruht das Gedächtnis von YGGDRASIL selbst — Dokumentation, Backups, Inventar.

[Dokumentation](ODIN/documentation.md){: .world-link } · [Backup](ODIN/backup.md){: .world-link } · [Inventar](ODIN/inventory.md){: .world-link } · [Wartung](ODIN/maintenance.md){: .world-link }

---

### 🐦‍⬛ RAVEN — Die Boten
Die kleinen, flinken Boten des Weltenbaums. Funk, Wellen und Hardware-Experimente über den Äther.

[Raspberry Pi](RAVEN/raspberry-pi.md){: .world-link } · [Shortwave](RAVEN/shortwave.md){: .world-link } · [SDR](RAVEN/sdr.md){: .world-link }

---

## 🚀 Aktive Missionen

| Mission | Welt | Status | Letzte Aktivität |
|---------|------|--------|------------------|
| SOC Lab Aufbau | HEIMDALL | 🟢 Aktiv | Wazuh Agents deployed |
| Wazuh SIEM Tuning | HEIMDALL | 🟢 Aktiv | Rules anpassen |
| Suricata IDS | HEIMDALL | 🟢 Aktiv | EveBox Alerts analysieren |
| AzuraCast Stream | MIDGARD | 🟢 Online | 24/7 Radio läuft |

---
## 🛡️ Datenschutz
[Datenschutz](datenschutz.md){: .world-link }

> *"Von den Wurzeln bis zu den Zweigen — alles verbunden, alles überwacht."*
